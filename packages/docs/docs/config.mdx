...
Overrides configuration shared by Webpack and Rspack.

The callback receives the current configuration and a context object containing `bundler`, which is either `'webpack'` or `'rspack'`.

Use this option only for changes that are compatible with both bundlers. Bundler-specific plugins and options should use `overrideWebpackConfig()` or `overrideRspackConfig()` instead.

```ts twoslash title="remotion.config.ts"
import {Config} from '@remotion/cli/config';
// ---cut---
Config.overrideBundlerConfig((currentConfiguration, {bundler}) => {
  return {
...
The shared override runs before the selected bundler's specific override.

## `overrideWebpackConfig()`<AvailableFrom v="1.1.0" />

See [Webpack and Rspack](/docs/bundlers) for usage and examples.
...
If `overrideBundlerConfig()` is also set, the shared override runs first.
...
## `addElementLibrary()`<AvailableFrom v="4.0.517" />

Adds an externally hosted Element library to Remotion Studio.
...
The URL must be an absolute HTTP or HTTPS URL. Remotion Elements remains available without adding it to the configuration.

## `setMaxTimelineTracks()`<AvailableFrom v="2.1.10" />

Set how many tracks are displayed in the Studio timeline at most. This does not affect your video, only the number of tracks shown while previewing. From <AvailableFrom v="4.0.514" inline />, all tracks are available by default and the timeline is virtualized.
...
Available coding agents: `codex`, `cursor`, `copilot`, and `claude-code`.
...
Pass multiple ordered, non-overlapping ranges to render discontinuous sections.<AvailableFrom v="4.0.502" inline />
...
## `setGopSize()`<AvailableFrom v="4.0.466" />
...
import {Config} from '@remotion/cli/config';
// ---cut---
Config.setGopSize(239);
```

The [command line flag](/docs/cli/render#--gop) `--gop` will take precedence over this option.
...
## `setExperimentalKeepAudioContextAlive()`<AvailableFrom v="4.0.508" />

<ExperimentalBadge>
  <p>We are currently testing this prop. We might either remove the functionality or rename the prop to a stable name.</p>
</ExperimentalBadge>

<Options id="experimental-keep-audio-context-alive" />
<br />
<br />

Keeps the shared [`AudioContext`](https://developer.mozilla.org/en-US/docs/Web/API/AudioContext) running while the Remotion Studio is paused. Pausing lowers the gain to zero rather than suspending the context, which can reduce latency when resuming playback.
...
Changing this option while the Studio is running reloads the page. The `--experimental-keep-audio-context-alive` command line flag will take precedence over this option.
...
## `setEnableCancellation()`<AvailableFrom v="4.0.515" />

<Options id="enable-cancellation" />
<br />
<br />
...
import {Config} from '@remotion/cli/config';
// ---cut---
Config.setEnableCancellation(true);
```

The `--enable-cancellation` command line flag will take precedence over this option.
